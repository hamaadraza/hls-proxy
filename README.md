...
|---|---|---|
| `BASE_URL` | *(request `Host`)* | Public origin that rewritten URLs point at, e.g. `https://hls-proxy.xyz`. Falling back to the request host means local runs need no config. |
